...
// Get booking details including filter question answers (admin or responsible manager)
router.get('/:id/details', authenticateAdminOrManager, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        // Check if user has access to this infrastructure
        if (!await hasInfrastructureAccess(req, res, booking.infrastructure_id)) return;
